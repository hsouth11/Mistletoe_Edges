...
Summary tables and figures are created in each section. The primary starting 
object for the script is the output of the '05_seed_load.Rmd' script 
(trees_sl.csv). 
...
#Check variables in tree data again
str(trees)
```

##Subset data to objects for different comparisons
There are a couple things that need to be considered to ensure that fair 
comparisons are made in this script. We want to make sure that whenever we are 
comparing two sites, we are comparing trees from equivalent areas. 
(1) Regen transects varied in length between sites.  
(2) At two sites (mi_1) and (mk_3) regen transect length varied within a site.

Before generating mmultiple data objects, also want to calculate common 
variables to avoid repetitive work. 

Calculate basal area (m^2) from dbh. https://en.wikipedia.org/wiki/Basal_area
```{r}
...
climate <- read_csv('./data/cleaned/hdm_sites_clim_data.csv') #raw data from climateBC
...
Define climate variables that were in Barrett et al. (2012). All of these 
variables have a link to HDM biology. Because Barret et al. (2012) used 
different climate data from a different source (i.e., not ClimateBC or 
ClimateNA), there are some cases where the variables defined here are not 
exactly equivalent. However these are only meant to be qualitative comparisons
so these inconsistencies are unlikely to be an issue. 
```{r}
#1 Growing degree days
#Defined over the whole year and for the growing season (spring to autum)
climate <- climate %>% rowwise() %>% mutate(DD5_gs = sum(c_across(DD5_sp:DD5_at)))
select(climate, DD5, DD5_gs)

#2 Growing season radians
#Mean annual radiation also included
...
###David R. (BC MoF collaborator) suggested presenting precipitation over season. 
###The link to precip is that rain or snow can pull HDM seeds off of foliage. 
climate <- climate %>% 
...
a larger difference? And do Pacific Highway sites have a larfer difference?
```{r}
...
```
Answer is yes to both questions. This aligns with what would be expected based
just on BEC classification. Pacific Highway sites are in the CWHvm1 (very wet, 
maritime) but verging on the CWHvh (very wet, hypermaritime). Its hard to know
whether this difference is significant, but the trend is there: Pacific 
Highway sites have lower infection levels in regen (analyszed below) and 
more winter precipitation; the lower infection levels could plausibly be
explained by precip pulling seeds off of foliage. 
...
slopes for the fall line. For both of those sets we measured slope at multiple 
...
#Add declination
x <- site %>% select(site_id, declination)
...
slope <- slope %>% 
  mutate(reg_az_corr = case_match(fl_az_device,
                                  "compass" ~ reg_az,
                                  "rangefinder" ~ ((reg_az) + declination)%%360))
...
Need to assemble a summary tables for thesis/publication that generally 
describes the forest at the sites. 
...
Load that. At some sites multiple vri polygons overlap the mature component. In 
...
the crown volume script and are stored in a table called cd_ht_wide.csv. Load 
that and calculate the height difference. 
```{r}
cd_ht <- read_csv(here("./data/workflow/cd_ht_wide.csv"))
head(cd_ht)
str(cd_ht)

#Sort by site id, order that is used throughout scripts
cd_ht <- cd_ht %>% arrange(site_id)
...
infection towards bigger or smaller trees. Generally speaking with these 
exploratory analyses, we are trying to assess whether the mature components are 
similar or different infection sources. 
...
#trees in three of the four levels (Unknown, No, Yes)
...
###The two spp are pretty similar but don't make up that much of the basal
###area generally. 
...
###what the range of dbh is. Likely that they are what makes up most of the
###basal area. 
mature %>% 
...
those the percent infection by each metric. General trends:
  - percent infected (0-15m): mean = 49, range = 1 - 80
  - percent infected (0-50m): mean = 24, range = 0 - 56
...
impacts are occurring in dominant/codominant trees. Past literature identifies
DMR >= 3 as the cutoff where growth impacts occur. 
```{r}
#How many are there? 
#All within 15m
regen_hw_dmr3 <- regen %>% 
