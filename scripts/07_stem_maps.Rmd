...
the output of the '05_seed_load.Rmd' script (trees_sl.csv) and the cleaned 
version of the trimble gps points (hdm_trimb_points.csv). 

The {sf} and {sp} packages are used to process the spatial data and the {tmap}
package is used to make the actual plots. {tmap} has a similar syntax to ggplot 
and a similar logic to ArcGIS. You can add layers and slowly build up a map. 
...
trees <- st_as_sf(trees, coords = c("x_utm", "y_utm"), crs = 3005)
...
Format transect dataset
```{r}
#Sort by site_id
tran <- tran %>% arrange(site_id)
```
...
site <- site %>% 
  ...
  unite(col = id_bec_ag_f, sep = ", ", 
        c("site_id", "bec_z_sz", "yr_since_harv"), remove = FALSE) %>% 
...
#Define a list of centre coordinates corresponding to each site. Use the 
...
trees_rot <- as.data.frame(trees_rot) %>% rename(x_utm_rot=V1, y_utm_rot=V2)

trimb_rot <- do.call(rbind, trimb_rot) 
trimb_rot <- as.data.frame(trimb_rot) %>% rename(x_utm_rot=V1, y_utm_rot=V2)

#Then associate back to the attribute data associated with each point
trees_att <- trees %>% st_drop_geometry()
trees_rot <- trees_att %>% mutate(x_utm_rot=trees_rot$x_utm_rot, 
                                  y_utm_rot=trees_rot$y_utm_rot) 
trees_rot <- st_as_sf(trees_rot, coords = c("x_utm_rot", "y_utm_rot"), crs=3005)

trimb_att <- trimb %>% st_drop_geometry()
trimb_rot <- trimb_att %>% mutate(x_utm_rot=trimb_rot$x_utm_rot, 
                                  y_utm_rot=trimb_rot$y_utm_rot) 
trimb_rot <- st_as_sf(trimb_rot, coords = c("x_utm_rot", "y_utm_rot"), crs=3005)
```
...
# pdf("./figures/stem_map_dmr.pdf", width=8.5, height=11)
# p 
# dev.off()
```
...
#Save the plot
# tmap_save(p, filename = "./figures/stem_map_set1_dmr_sl.svg",
...
#Save the plot
# tmap_save(p, filename = "./figures/stem_map_set2_dmr_sl.svg",
#           width = 6.5, height = 7.5, units = "in")
```
...
#Save the plot
# tmap_save(p, filename = "./figures/stem_map_defence_dmr_sl.svg",
...
p <- tm_shape(adjusted_fp_rot, is.master = TRUE) + #define map extent using site footprints
...
#add tree points
  tm_shape(trees_rot) + 
  tm_symbols(size = "dbh", scale = 1.5, col="dmr_f_graph", palette = color_dmr, 
...
# tmap_save(p, filename = "./figures/stem_map_wifdwc.pdf",
