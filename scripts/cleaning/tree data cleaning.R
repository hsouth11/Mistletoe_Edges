...
# Read in data
regen <- read_csv("./data/raw/regen comp_master_10May2024.csv")
mature <- read_csv('./data/raw/mature comp_master_10May2024.csv')
...
# plot_id: unique id of stem mapping plot; integer
...
# dec_deg: column for declination at the site, not filled in;
...
### if spp!=Hw, then dmrs = 0 and all other hdm variables are "-"
### if hdm_na = N, then dmrs = 0 and all other hdm varaiables are "-"
### if dmr = sum or dmrs, except when dmr = 0, hdm_pa = Y and b_lc = Y (i.e. the tree is only infected below live crown)
...
########## LOGICAL CHECKS ########## 
#Essential data: all trees should have tree_id, site_id, plot_id, spp, status, dbh, hdm_pa, crown class, and crown cond
sum(is.na(trees$tree_id)) #check there are none in tree_id
...
#One tree with dbh<9, tree_id = m385. Data entry error should be dbh = 14.0. Fixed in raw data. Checked a few large dbhs (m476, m472, m478, m30). m476 was a data entry error (should be r=17.9 (not 179)) others good. Fixed in raw data.   

#How many transects are there? Should be 24 (3 tranects/site * 8 sites)
trees %>% filter(tree_type=="regen") %>% pull(plot_id) %>% unique() %>% length() #24

########## Cleaning HDM variables ########## 
##### RULES
### Rule 1: if spp!=Hw, then dmrs = NAs and all other hdm variables are "-"
### Rule 2: if spp=Hw, it can can have hdm_pa = Y, N or U (unknown for unassessable trees, e.g. snags)
### Rule 3: if spp=Hw, it can have dmr by crown third= NA (dead trees, snags), 0, 1 or 2
###### Special cases: 
### Rule 4: if spp=Hw, status= LS, LL, LF or RD (i.e. its alive or recently dead) then hdm factor variables and dmrs are defined
### Rule 5: if spp=Hw, and status=SN or DS (i.e. its been dead for a while) then hdm_pa can be Y, N, U but dmrs = NA and all other hdm factors = "-" (i.e. dmr and hdm factors were not assessed for these trees)
...
# When spp = Hw, status = DS or SN, dmr will = NA. This isn't quite representative because it could be infected. Call this: DI (dead, infected) if infected and DU (dead uninfected) if uninfected.
# To account for this for these cases, we will make another tree level dmr variable, that is a factor

### Rule 1: if spp!=Hw, then dmrs = NA and all other hdm variables are "-"
trees %>% filter(spp!="Hw") %>% select(spp, hdm_pa) %>% group_by(spp) %>% count(hdm_pa)
#Fd is the only other species where we observed HDM infection, but we were unsure about it and 
# can note that in our discussion. 

#Set factors = "-"
trees <- trees %>% mutate(across(c(hdm_pa, b_lc, broom_pa, broom_pos, stem_pa), ~ 
                                   case_when(spp!="Hw" ~ "-", TRUE ~ .))) %>% 
  mutate(across(c(hdm_pa, b_lc, broom_pa, broom_pos, stem_pa), ~as.factor(.)))
trees %>% filter(spp!="Hw") %>% select(spp, hdm_pa) %>% group_by(spp) %>% count(hdm_pa) # Check, looks good. 

#Set dmrs = NA
trees <- trees %>% mutate(across(dmr_l:dmr_u, ~ case_when((spp!="Hw") ~ NA, TRUE ~ .)))
trees %>% filter(!spp=="Hw") %>% 
...
trees %>% filter(spp=="Hw") %>% select(hdm_pa) %>% summary() #Y, N and -. 
# Want it to be Y, N and U. Only six trees with "-"

#Lets look at those. All are snags, so - indicates un assessable. Lets redefine them to U
trees %>% filter(spp=="Hw" & hdm_pa=="-")
trees <- trees %>% mutate(hdm_pa = case_when((spp=="Hw" & hdm_pa=="-") ~ "U", 
                                             .default = hdm_pa)) %>% mutate(hdm_pa = factor(hdm_pa))
trees %>% filter(spp=="Hw") %>% select(hdm_pa) %>% summary() #check, looks good


### Rules 3-5
# Case 1: status = LS, LL, LF, RD (live and recently dead trees)
trees %>% filter(spp =="Hw" & status %in% c("LS", "LL", "LF", "RD")) %>% 
  group_by(status) %>% summarise(hdm_pa = table(hdm_pa)) 
...
levels(trees$hdm_pa)
# So LF all uninfected, LL and LS both infected and uninfected, RD trees all infected

# Set hdm factor variables equal to "-' and dmr = 0, where spp=Hw, hdm_pa=N and status = LS, LL, LF, RD
trees <- trees %>% mutate(across(c(b_lc, broom_pa, broom_pos, stem_pa), ~ 
                                   case_when((spp=="Hw" & hdm_pa=="N" & 
                                              status %in% c("LS", "LL", "LF", "RD")) ~ "-", TRUE ~ .))) %>% 
                  mutate(across(c(b_lc, broom_pa, broom_pos, stem_pa), ~as.factor(.)))

trees %>% filter(spp=="Hw" & hdm_pa=="N" & status %in% c("LS", "LL", "LF", "RD")) %>% 
  select(b_lc, broom_pa, broom_pos, stem_pa) %>% summary()
#Check, looks good

#Set dmrs = 0 if spp=Hw and hdm_pa=N
trees <- trees %>% mutate(across(dmr_l:dmr_u, ~ case_when((spp=="Hw" & hdm_pa=="N"
                                                           & status %in% c("LS", "LL", "LF", "RD")) ~ 0, TRUE ~ .)))

trees %>% filter(spp=="Hw" & hdm_pa=="N" & status %in% c("LS", "LL", "LF", "RD")) %>% 
  select(dmr_l:dmr_u) %>% summary() # looks good

# Case 2: spp=Hw and status=SN or DS (i.e. its been dead for a while)
trees %>% filter(spp =="Hw" & status %in% c("DS", "SN")) %>% 
  group_by(status) %>% summarise(hdm_pa = table(hdm_pa)) 
# gives you a table summarizing # of trees in each hdm_pa level by status. Unfortunately, it doesn't explicitly list the level of the hdm_pa in the table so list that explictly to help read the table. 
levels(trees$hdm_pa)
# So DS contains both infected and uninfected and SN contains uninfected, infected and unassessable
...
trees <- trees %>% mutate(across(c(b_lc, broom_pa, broom_pos, stem_pa), ~ 
                                   case_when((spp=="Hw" & status %in% c("DS", "SN")) ~ "-", TRUE ~ .))) %>% 
                   mutate(across(c(b_lc, broom_pa, broom_pos, stem_pa), ~as.factor(.)))
trees %>% filter(spp=="Hw" & status %in% c("DS", "SN")) %>% 
  select(hdm_pa, b_lc, broom_pa, broom_pos, stem_pa) %>% summary() #Check, looks good. 

#Set dmrs = NA
trees <- trees %>% mutate(across(dmr_l:dmr_u, ~ case_when((spp=="Hw" & status %in% c("DS", "SN")) ~ NA, TRUE ~ .)))
...
trees <- trees %>% 
  mutate(dmr_f = case_when(spp!="Hw" ~ "-", # not Hw, give "-"
                           (spp=="Hw" & hdm_pa=="Y" & b_lc=="Y" & dmr==0) ~ "IBLC", # live or recently dead, infected b lc
                           (spp=="Hw" & status %in% c("DS", "SN") & hdm_pa=="Y") ~ "DI", # dead a while, infected
                           (spp=="Hw" & status %in% c("DS", "SN") & hdm_pa %in% c("N", "U")) ~ "DU", # dead a while uninfected
