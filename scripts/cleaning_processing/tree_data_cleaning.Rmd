...
into a single dataframe and cleans the variables. There is a complementary 
data dictionary in "./data/data_dictionary/dict_trees.csv"
...
infected, 2 = >50% infected. Assessed for trees with "hdm_pa = Y" and 
status = "LS or LL or LF". For dead susceptible species ("spp = Hw or Ba") and 
all non-susceptible species ("spp != Hw or Ba") it is NA. 

Check that:
(1) Columns are well defined (0, 1 or 2) for all live, infected hemlock and 
amabilis fir
(2) It is NA for live, uninfected hemlock and ambilis fir, dead hemlock and 
amabilis fir and all other species
...
```{r}
#Check all cases are NA in each DMR columns
#Some trees in this category that have non-NA values
trees %>% filter(spp %in% c("Hw", "Ba") &
                    status %in% c("LS", "LL", "LF") &
                    hdm_pa == "N") %>%
  select(contains("dmr")) %>% 
...
#Look at those trees
#Probably same four that were problem in the HDM factor variables
trees %>% filter(spp %in% c("Hw", "Ba") &
                    status %in% c("LS", "LL", "LF") &
                    hdm_pa == "N" &
