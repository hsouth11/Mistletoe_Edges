...
Export the table of heights for future use
```{r}
#Use the long table but add a column identifying which data source to use
...
#Export
# write_csv(top_ht_g, here("./data/workflow/top_ht.csv"))
```
...
#Export
# pdf(here("./figures/htlc_ht.pdf"), width = 10, height = 7)
# p2
# dev.off()
```
...
#Combine median, min and max for each variable into single columns
tbl_cv <- tbl_cv %>%
  pivot_longer(
  ...
  mutate(combined = paste0(median, " [", min, "-", max, "]")) %>% 
  select(tree_type, crown_class_2, variable, combined) %>% 
  pivot_wider(id_cols = c(tree_type, crown_class_2),
              names_from = variable,
...
tbl_cv <- tbl_cv %>%
  relocate(ht_fac, .before = height_cv_est) %>% 
  mutate(ht_fac = round(ht_fac, 2))
