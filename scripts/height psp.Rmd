...
########################################
#########Get missing height data from vri
...
levels(vri$ID)
vri <- vri %>% 
  filter(ID %in% c("mi_2_hdm_m", "mi_2_hdm_r", "ph_2_hdm_m"))
head(vri)

#Create two columns to align with hdm_ht dataset:
#crown class (call these codominant)
#height_m (height in meters)
vri <- vri %>% 
  mutate(height_m = case_when(SPECIES_CD_1 == "HW" ~ PROJ_HEIGHT_1,
                              SPECIES_CD_2 == "HW" ~ PROJ_HEIGHT_2, 
                              .default = NA),
         crown_class = "C")
head(vri)

#Two vri polygons representing mi_2 regen.
#Treat that by taking average
vri <- vri %>% group_by(ID, crown_class, tree_type) %>% 
  summarise(height_m = mean(height_m))
head(vri)

#Combine field measured and vri heights into a single dataset
common_vars <- intersect(names(ht_hdm), names(vri))
ht_hdm <- ht_hdm %>% select(all_of(common_vars))
vri <- vri %>% select(all_of(common_vars))
ht_hdm <- rbind(ht_hdm, vri)

########Summarize field measured heights by site component
#First filter to just codominant and dominant heights
...
#Look at variability in codominant/dominant height by site
ggplot(ht_hdm, aes(x = ID, y = height_m)) + 
  geom_point(aes(color=crown_class)) + 
  geom_boxplot(fill=NA)+
  theme(axis.text.x = element_text(angle=90))

#generally, mature components have more height variability
#vri heights are represented by one number and so will have no standard
#deviation. Deal with this by assigning them the average of the mature/regen
#components respectively

#Summarise height data for each site component with three variables: mean, 
#median and standard deviation
ht_hdm_sum <- ht_hdm %>% group_by(ID, tree_type) %>% 
  summarise(mean_ht = mean(height_m),
            median_ht = median(height_m),
            sd_ht = sd(height_m))
summary(ht_hdm_sum)
missing_sd <- ht_hdm_sum %>% filter(is.na(sd_ht)) %>% pull(ID)
reg_sd <- mean(ht_hdm_sum$sd_ht[ht_hdm_sum$tree_type == "regen"], na.rm = TRUE)
mat_sd <- mean(ht_hdm_sum$sd_ht[ht_hdm_sum$tree_type == "mature"], na.rm = TRUE)

#Assign group standard deviation to vri sites with only one measurement
ht_hdm_sum <- ht_hdm_sum %>% 
  mutate(sd_ht = case_when(ID %in% missing_sd & 
                              tree_type == "mature" ~ 
                              mat_sd,
                            ID %in% missing_sd & 
                              tree_type == "regen" ~ 
...
#Split this dataset by tree type
ht_hdm_reg <- ht_hdm_sum %>% filter(tree_type == "regen")
ht_hdm_mat <- ht_hdm_sum %>% filter(tree_type == "mature")
#Good. This is final dataset of height metrics for integration into PCA
```
...
#########################
######Filter to sites by proximity, bec zone and measurment completeness
...
idcheck <- t_psp %>% group_by(SITE_IDENTIFIER) %>% 
...
#are calculated in last step, then use seperate mature/regen datasets. 
...
#Create three variables to integrate into PCA workflow from codominant and 
#dominant field measured Hw heights: median, mean, standard deviation

#psp heights for dataset comparing against hdm regen site components
ht_psp_reg <- t_psp_reg %>% 
  filter(CR_CL %in% c("C", "D") & 
           SPECIES == "HW" &
           HEIGHT_SOURCE == "Field measured") %>%
  group_by(CLSTR_ID) %>% 
  summarise(n_d_c_ht = n(),
            mean_ht = mean(HEIGHT), 
            median_ht = median(HEIGHT),
            sd_ht = sd(HEIGHT))
summary(ht_psp_reg)

#Check if there are any NA values
any(is.na(ht_psp_reg$mean_ht))
any(is.na(ht_psp_reg$median_ht))
any(is.na(ht_psp_reg$sd_ht)) #NA values here, likely from site visits with only
#one measurement
sum(is.na(ht_psp_reg$sd_ht)) #eleven sites visits
filter(ht_psp_reg, is.na(sd_ht))

#Filter to sites with at least 5 height measurements
ht_psp_reg <- ht_psp_reg %>% filter(n_d_c_ht >=5)

#Create ID variable used to join with pca dataframe
ht_psp_reg <- ht_psp_reg %>% mutate(site_type = "psp_r") %>% 
  unite(col = ID, sep = "_", c("CLSTR_ID", "site_type"), remove = FALSE)
head(ht_psp_reg)

#psp heights for dataset comparing against hdm mature site components
ht_psp_mat <- t_psp_mat %>% 
  filter(CR_CL %in% c("C", "D") & 
           SPECIES == "HW" &
           HEIGHT_SOURCE == "Field measured") %>%
  group_by(CLSTR_ID) %>% 
...
#Check if there are any NA values
any(is.na(ht_psp_mat$mean_ht))
any(is.na(ht_psp_mat$median_ht))
any(is.na(ht_psp_mat$sd_ht)) #NA values here, likely from site visits with only
#one measurement
sum(is.na(ht_psp_mat$sd_ht)) #eleven sites visits
filter(ht_psp_mat, is.na(sd_ht))

#Filter to sites with at least 5 height measurements
ht_psp_mat <- ht_psp_mat %>% filter(n_d_c_ht >=5)

#Create ID variable used to join with pca dataframe
ht_psp_mat <- ht_psp_mat %>% mutate(site_type = "psp_m") %>% 
  unite(col = ID, sep = "_", c("CLSTR_ID", "site_type"), remove = FALSE)
head(ht_psp_mat)

#Update tree level psp dataset to refelct site visits filtered out for not
#having complete height measurements
t_psp_reg <- t_psp_reg %>% semi_join(ht_psp_reg, by = "CLSTR_ID")
t_psp_mat <- t_psp_mat %>% semi_join(ht_psp_mat, by = "CLSTR_ID")

#Leaves us with this amount for PCA: 
length(unique(t_psp_reg$CLSTR_ID))
length(unique(t_psp_mat$CLSTR_ID))
```

#Generate datasets for PCA
```{r}
...
#Optionally, I've tried integrating
#Combination 1: site index and age of the leading trees in the 
#codominant/dominant canopy layers. Problematic because you lose a lot of the
#psp samples. 
#Combination 2: mean, median and sd of height of codominant/dominant Hw trees. 
#This combination retains more psp samples and is aimed at finding pairs of
#sites where the dimensions of trees are the same. Site characteristics are 
#emphasized less in this approach (following Déa's suggestion)

#Not using min(dbh) because the minimum is set by the dbh cutoff used in the
#protocols for measuring both hdm sites and the psp sites
...
#Calculate the dbh metrics for the hdm data sets
...
# #Add optional variable combination 1: age and site_index data
...
#Add optional variable combination 2: height metrics of codominant/dominant Hw
#Combine vri and psp height datasets
common_vars <- intersect(names(ht_hdm_reg), names(ht_psp_reg))
ht_hdm_reg <- ht_hdm_reg %>% select(all_of(common_vars))
ht_psp_reg <- ht_psp_reg %>% select(all_of(common_vars))
ht_reg <- rbind(ht_hdm_reg, ht_psp_reg)

common_vars <- intersect(names(ht_hdm_mat), names(ht_psp_mat))
ht_hdm_mat <- ht_hdm_mat %>% select(all_of(common_vars))
ht_psp_mat <- ht_psp_mat %>% select(all_of(common_vars))
ht_mat <- rbind(ht_hdm_mat, ht_psp_mat)

#Add height metrics to pca dataframes
pca_stat_reg <- left_join(pca_stat_reg, ht_reg, by = "ID")
pca_stat_mat <- left_join(pca_stat_mat, ht_mat, by = "ID")

#The numeric variables in these datasets should be the final set of variables 
#for PCA. Look at them:
str(pca_stat_reg)
str(pca_stat_mat)
...
max_dbh_reg <- max(pca_stat_reg$max_dbh[pca_stat_reg$site_type == "hdm_r"])
max_dbh_reg
...
range(pca_stat_mat$mean_dbh[pca_stat_mat$site_type == "hdm_m"])
range(pca_stat_mat$median_dbh[pca_stat_mat$site_type == "hdm_m"])
range(pca_stat_mat$max_dbh[pca_stat_mat$site_type == "hdm_m"])
...
site_reg_bec <- site %>% select(site_id, bec) %>% 
  rename(orig_id = site_id)
bec <- rbind(v_psp_reg_bec, site_reg_bec)
#Join this to the pca.stat dataframe
pca_stat_reg <- left_join(pca_stat_reg, bec, by = "orig_id")

#Plot the PCA plot again, this time coloured by bec
pca_stat_reg <- pca_stat_reg %>% 
  mutate(site_id_lab = if_else(site_type == "hdm_r", orig_id, NA))
...
cand_2 <- pca_stat_reg %>% 
  ...
  arrange(!!sym(i)) %>% 
  filter(ID == i | (site_type =="psp_r" & !ID %in% cand_1$ID)) %>% 
  slice(2:4) %>% 
  mutate(restr ="wo_bec")
...
#Bind the datasets
dbh_reg <- rbind(select(regen_15, ID, orig_id, spp, PHF_TREE, dbh, dbh_bin),
                 select(t_psp_reg, ID, orig_id, spp, PHF_TREE, dbh, dbh_bin))
...
dbh_reg <- left_join(dbh_reg, select(pca_stat_reg, ID, bec), 
                     by = "ID")
dbh_reg <- dbh_reg %>% 
  unite(col = id_bec, sep = ";", c("orig_id", "bec"), remove = FALSE)

#Summarize the number of trees in each dbh bin
dbh_reg_bin <- dbh_reg %>% group_by(ID, id_bec, hw, dbh_bin) %>% 
...
hdm_r_ids
focal_hdm_site <- reg_candidates %>% filter(hdm_site == hdm_r_ids[11])
plot_reg <- dbh_reg_bin %>% filter(ID %in% focal_hdm_site$ID)
...
#   mutate(site_type = "hdm_r") %>% 
#   unite(col = ID, sep = "_", c("site_id", "site_type"), remove = FALSE)
# t_ht_psp_reg <- t_ht_psp_reg %>% mutate(site_type = "psp_r") %>% 
#   unite(col = ID, sep = "_", c("CLSTR_ID", "site_type"), remove = FALSE) %>% 
#   rename(dbh = DBH, height_m = HEIGHT)
# 
# #bind the two datasets
# t_ht_reg <- rbind(select(t_ht_hdm_reg, ID, dbh, height_m),
#                  select(t_ht_psp_reg, ID, dbh, height_m))
# 
# ggplot(filter(t_ht_reg, ID %in% (reg_candidates$ID[13:16])),
#        aes(x=dbh, y=height_m, color = ID)) + 
#   geom_point()
```
...
#Rename variables so they are consistent between datasets
#shared across the datasets
mature_comp <- mature_comp %>% mutate(site_type = "hdm_m") %>% 
  unite(col = ID, sep = "_", c("site_id", "site_type"), remove = FALSE) %>% 
  rename(orig_id = "site_id")
t_psp_mat <- t_psp_mat %>% mutate(site_type = "psp_m") %>% 
  ...
  rename(orig_id = CLSTR_ID, dbh = DBH, spp = SPECIES)
#Bind the datasets
dbh_mat <- rbind(select(mature_comp, ID, orig_id, spp, PHF_TREE, dbh, dbh_bin),
