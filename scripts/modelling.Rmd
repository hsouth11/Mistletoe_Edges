...
#Dataset 8: climate
clim <- read_csv(here("./tables/site_climate.csv"))
...
x <- reg_cs %>% 
  select(site_id, reg.nstem_ha_15, reg.tot_ba_m2ha_15, reg.p_ba_hw)
...
because it is colinear with distance from the edge. So as an alternative, 
include some proxies for the inoculum load in the mature component at the site
level. Include two proxies here: seed production and basal area of infected 
trees. We would expect seed production to perform better because it
...
Finished assembling the dataframe. Filter to live Hw trees within 50m of the 
edge and reformat variables to the appropriate class.
```{r}
...
modelling %>% filter(sim_tree == "N") %>% nrow() #548 measured trees
modelling %>% filter(sim_tree == "Y") %>% nrow() #550 simulated trees
...
col_dmr_7 <- c("#A6CEE3", "hotpink", "#FEEDDE", "#FDBE85", "#FD8D3C", 
                "#E6550D", "#A63603")
...
Plot dmr against continuous predictor variables
```{r}
#SEED LOAD
...
ggplot(modelling, aes(x = sl_i_tot_rm, 
  ...
  labs(y = "Adapted DMR", x = "Seed load proxy") +
  theme_classic()

#DISTANCE FROM THE EDGE
#Boxplot comparing dmr to distance from the edge
ggplot(modelling, aes(x = dist_y_h, 
                      y = dmr_f)) + 
  geom_jitter(height = 0.2) +
  geom_boxplot(fill = NA, outliers = FALSE) +
  labs(y = "Adapted DMR", x = "Distance from edge") +
  theme_classic()
...
ggplot(modelling, aes(x = dbh, 
  ...
  geom_jitter(height = 0.2) +
  geom_boxplot(fill = NA, outliers = FALSE) +
  labs(y = "Adapted DMR", x = "DBH (cm)") +
  theme_classic()
```

Plot dmr against crown class
```{r}
...
#Join these tables, then calculate the proportion of trees in each dmr class 
#within each crown class
x <- left_join(x, x1, by = "crown_class")
x <- x %>% 
  mutate(prop_trees = n_trees/tot_trees)

#Check these sum to 1
x %>% group_by(crown_class) %>% summarise(sum(prop_trees))

#Add fake values to the x1 dataframe so it can be added to the plot to show the
#total sample size in each crown class
x1 <- x1 %>% 
  mutate(prop_trees = 1.05,
         dmr_f = NA)

#Plot this: 
ggplot(x, aes(x = crown_class, y = prop_trees, fill = dmr_f)) + 
  geom_bar(position = position_stack(reverse =  TRUE), stat = "identity") +
  scale_fill_manual(values = dmr_colour, 
                    na.translate = FALSE, 
  ...
  geom_text(data = x1, aes(x=crown_class, 
                           ...
                           label = tot_trees),
            show.legend = FALSE) + 
  labs(x = "Crown class", y = "Proportion of trees", fill = "Adapted DMR") + 
  theme_classic()
```
...
##Univariate models
Build univariate models testing the significane of each of the tree level 
...
Okay, all interactions are significant. Does a model with all interactions
converge?
```{r}
#Model 15
...
Okay, seems like all three interaction terms are needed. Build all three models
with two interactions and compare AICs. 
```{r}
#Model 16
...
#interaction. Models 16, 17 and 18 have two. Model 15 has all three. 
##Interpretation: Model 16 (seed load x distance and seed load x dbh 
##interactions) performs best. Go with this one. 
AIC(m12); AIC(m13); AIC(m14); AIC(m16); AIC(m17); AIC(m18); AIC(m15)
```
...
In those tests, only distance from the edge failed to meet the proportional 
odds assumption. Biljana Stojkova from the Stats group suggested also assessing
this assumption graphically. We do this by plotting the logit(odds) of each
of the submodels against a given predictor variable. Each submodel models the
cumulative logit odds of P<= j and we are assuming that the effect of each 
predictor is the same across each submodel, which is the same as saying across 
each j. In practice, the lines would be parallel if the assumption is met and
and no parallel if it is not. 
```{r}
...
ranef_nom_mod <- data.frame(ranef_m22 = m22$ranef,
                            ranef_m23 = m23$ranef,
                            ranef_m24 = m24$ranef,
                            ranef_m25 = m25$ranef,
                            ranef_m26 = m26$ranef,
...
x <- x %>% 
  mutate(lo_0_m22 = (coef_m22[1] + coef_m22[3]*dist_y_h.s) - 
...
x <- x %>% 
  mutate(cp_0_m22 = plogis(lo_0_m22),
...
#Plot the log odds against distance from the edge. Are the lines parallel? 
...
po_m22 <- ggplot(x2, aes(x = dist_y_h.s, y = lo_val, colour = lo_level)) + 
  geom_point(alpha = 0.3) + geom_smooth(se = FALSE) + theme_classic()
...
ggarrange(po_m22, po_m23, po_m24, po_m25, po_m26, 
          ncol = 3, nrow = 2, 
          common.legend = TRUE)
```
...
##Interpretation: high values aren't observed for trees with dbh > 
...
#Backtransform distance from the edge and seed load so it is on the original
#scale
pr1 <- pr1 %>% 
...
levels(pr1$group)
pr1 <- pr1 %>%
  mutate(sl = case_when(group == "-0.396160252046031" ~ "0",
                        group == "1.2217669409474" ~ "moderate",
                        group == "4.45762132693426" ~ "high")) %>% 
...
pr1 <- pr1 %>%
  mutate(
    dbh_f = case_when(
      dbh == 6.8 ~ paste0("dbh = ", dbh, " (S)"),
      dbh == 10.9 ~ paste0("dbh = ", dbh, " (I)"),
      dbh == 20.2 ~ paste0("dbh = ", dbh, " (C/D)")))
unique(pr1$dbh_f)
pr1 <- pr1 %>% 
  mutate(dbh_f = factor(dbh_f, levels = c("dbh = 6.8 (S)", 
                                          "dbh = 10.9 (I)",
                                          "dbh = 20.2 (C/D)")))
levels(pr1$dbh_f)
...
pr2 <- pr2 %>%
  mutate(
    dist_f = case_when(
      dist_y == 5 ~ paste0("dist = ", dist_y, "m"),
      dist_y == 25 ~ paste0("dist = ", dist_y, "m"),
      dist_y == 45 ~ paste0("dist = ", dist_y, "m")))
unique(pr2$dist_f)
pr2 <- pr2 %>% 
  mutate(dist_f = factor(dist_f, levels = c("dist = 5m",
                                            "dist = 25m",
                                            "dist = 45m")))
levels(pr2$dist_f)
...
pr3 <- pr3 %>%
  mutate(
    dist_f = case_when(
      dist_y == 2.5 ~ paste0("dist = ", dist_y, "m"),
      dist_y == 7.5 ~ paste0("dist = ", dist_y, "m"),
      dist_y == 12.5 ~ paste0("dist = ", dist_y, "m")))
unique(pr3$dist_f)
pr3 <- pr3 %>% 
  mutate(dist_f = factor(dist_f, levels = c("dist = 2.5m",
                                            "dist = 7.5m",
                                            "dist = 12.5m")))
levels(pr3$dist_f)
...
pr3 <- pr3 %>%
  mutate(
    dbh_f = case_when(
      dbh == 6.8 ~ paste0(dbh, " (S)"),
      dbh == 10.9 ~ paste0(dbh, " (I)"),
      dbh == 20.2 ~ paste0(dbh, " (C/D)")))
unique(pr3$dbh_f)
pr3 <- pr3 %>% 
  mutate(dbh_f = factor(dbh_f, levels = c("6.8 (S)", 
                                          "10.9 (I)",
                                          "20.2 (C/D)")))
levels(pr3$dbh_f)
...
pr3 <- pr3 %>% 
  filter(!(dist_f == "dist = 7.5m" & sl > 400)) %>% 
  filter(!(dist_f == "dist = 12.5m" & sl > 200))

#Generate the plot
g_pr3 <- 
  ggplot(pr3, aes(x = sl, y = predicted, color = DMR)) + 
  geom_line(aes(linetype = dbh_f)) +
  facet_wrap(~dist_f, ncol = 3) +
  theme_classic() + 
...
g_re1 <- ggplot(ran_ef, aes(x = site, y = ranef)) + 
  geom_point() + 
  geom_linerange(aes(ymin = ci[, 1], ymax = ci[, 2])) + 
  geom_hline(yintercept = 0, color = "grey", linetype = 2) +
  geom_hline(yintercept = re, color = "darkred", linetype = 3) +
  theme_classic() + 
...
dataset where distance from the edge varies (0-50) and seed load and dbh are at
their means; scale so they match the model inputs; estimate the average (random
effect = 0) and extreme (random effect = 95% CI limits) randome effects; manually calcualte probabilities of each response; then plot. 
```{r}
...
ran_ef2 <- ran_ef2 %>% 
  mutate(ranef = c(rep(0, 11), rep(re[1], 11), rep(re[2], 11)),
         ranef_f = factor(c(rep("av site", 11), rep("5th perc site", 11), 
                     rep("95th perc site", 11)), 
                     levels = c("5th perc site", 
                                "av site", 
                                "95th perc site"))) 
...
g_re2 <- ggplot(x, aes(x = dist_y_h, y=prob)) + 
  geom_point(data = filter(x, ranef_f == "av site"), 
             aes(x = dist_y_h, y = prob, colour = DMR)) +
  ...
  labs(x = "Distance from the edge (m)",
       y = "Probability",
       linetype = "Random effect") +
  theme(legend.position = "right") 
g_re2
```

###Combine random effect plots
```{r}
g_re3 <- ggarrange(g_re1, g_re2, ncol = 2, widths = c(1,2),
          labels = c("A", "B"))
g_re3
```
