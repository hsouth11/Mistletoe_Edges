...
#Join this to the modelling dataframe
x <- mat_inf %>% 
  select(site_id, mat.inf_ba_m2_ha)
modelling <- left_join(modelling, x, by = "site_id")
```
...
Going to recreate the table above but with a bit more detail. Rows will be 
variables. Columns will be divisions of the dataset. Crown class is 
excluded because its not in the model in thesis. 
...
tb2 <- modelling %>% 
  summarise(n_tot = n(),
            ...
            dist_sd = sd(dist_y_h),
            dist_med = median(dist_y_h),
            dist_min = min(dist_y_h),
            dist_max = max(dist_y_h),
            sl_mean = mean(sl_i_tot_rm),
            sl_sd = sd(sl_i_tot_rm),
            sl_med = median(sl_i_tot_rm),
            ...
            dbh_min = min(dbh),
            dbh_max = max(dbh)) %>% 
  mutate(across(dist_mean:dbh_max, ~ format(round(.x, 1), nsmall=1)))

#Combine mean and sd into single cell, median and range into another
tb2 <- tb2 %>% 
  mutate(dist_mean_sd = paste0(dist_mean, " (", dist_sd, 
                            ")"),
         dist_med_rg = paste0(dist_med, " [", dist_min, ", ",
                              dist_max, "]"),
         sl_mean_sd = paste0(sl_mean, " (", sl_sd, 
                            ")"),
         sl_med_rg = paste0(sl_med, " [", sl_min, ", ",
                              sl_max, "]"),
         dbh_mean_sd = paste0(dbh_mean, " (", dbh_sd, 
                            ")"),
         dbh_med_rg = paste0(dbh_med, " [", dbh_min, ", ",
                              dbh_max, "]"))

#Drop the raw versions
tb2 <- tb2 %>% 
  select(starts_with("n"), contains("mean_sd"), contains("med_rg")) %>% 
  as.data.frame()

#Assemble the dataframe in the shape of its final version
names <- c("N", "N (measured)", "N (simulated)",
           "distance from edge (m)", "seed load (no unit)", 
           "DBH (cm)")
col1 <- c(tb2[1,1], tb2[1,2], tb2[1,3], tb2[1,4], tb2[1,5], tb2[1,6])
col2 <- c(NA, NA, NA, tb2[1,7], tb2[1,8], tb2[1,9])

tb3 <- data.frame("Variables" = names, "col1" = col1, "col2" = col2)
```
...
tb4 <- modelling %>%
  ...
  summarise(n_tot = n(),
            ...
            dbh_mean = mean(dbh),
            dbh_sd = sd(dbh)) %>% 
  mutate(across(dist_mean:dbh_sd, ~ format(round(.x, 1), nsmall=1)))

#Combine mean and standard deviation into single cells
#Drop the raw versions
tb4 <- tb4 %>% 
  mutate(dist_mean_sd = paste0(dist_mean, " (", dist_sd, 
                            ")"),
         ...
         dbh_mean_sd = paste0(dbh_mean, " (", dbh_sd, 
                            ")")) %>% 
  select(dmr_f, starts_with("n"), contains("mean_sd"))

#Transpose
tb4 <- tb4 %>% t()

#Rename columns and then remove row with column names
colnames(tb4) <- tb4[1,]
tb4 <- tb4[-1,]
```

Combine the two into a final table
```{r}
tb5 <- bind_cols(tb3, tb4)

#Export this. Can pretty it up in word
# write_csv(tb5, here("./tables/model_dtset_sum.csv"))
```
...
dmr_pred1 <- ggplot(modelling, aes(x = dist_y_h, 
  ...
  labs(y = "Adapted DMR", x = "Distance from edge (m)") +
  theme_classic() +
...
dmr_pred2 <- ggplot(modelling, aes(x = sl_i_tot_rm, 
  ...
  geom_jitter(height = 0.2) +
  geom_boxplot(fill = NA, outliers = FALSE) +
  labs(y = "Adapted DMR", x = "Seed load (no units)") +
  theme_classic() +
  theme(axis.text = element_text(size = 16),
        axis.title = element_text(size = 16))
dmr_pred2
...
#Plot this: 
dmr_pred4 <- ggplot(x, aes(x = crown_class, y = prop_trees, fill = dmr_f)) + 
...
#Export
# pdf(here("./figures/dmr_t_pred.pdf"), width = 10, height = 7)
# dmr_pred
# dev.off()
```
...
#Export
# pdf(here("./figures/mod_vars_pairs.pdf"), width = 7, height = 10)
# pairs_the
# dev.off()
```
...
each j. Visually this means that the distance between two logits on a graph 
(e.g. P <= 0 and P <= IBLC-2) whould be the same across the range of a 
predictor; or more simply, that the lines for these two logits should be 
parrallel. 
```{r}
...
prop_odds <- ggarrange(po_m22, po_m23, po_m24, po_m25, po_m26, leg,
          ncol = 3, nrow = 2)

#Export
# pdf(here("./figures/prop_odds.pdf"), width = 10, height = 7)
# prop_odds
# dev.off()
```
...
effect = 0) and extreme (random effect = 95% CI limits) random effects; 
manually calcualte probabilities of each response; then plot. 
```{r}
...
g_re2 <- ggplot(x, aes(x = dist_y_h, y=prob)) + 
  ...
  theme(legend.position = "right",
        ...
        legend.box.background = element_rect(fill = "lightgrey", colour = NA),
        legend.key = element_rect(fill = "lightgrey", color = NA),
        legend.title = element_text(size = 20),
        legend.text = element_text(size = 16),
        axis.text = element_text(size = 16),
...
```{r}
g_re3 <- ggarrange(g_re1, g_re2, ncol = 2, widths = c(1,2.5),
          labels = c("A", "B"), font.label = list(size = 20))
g_re3

#Export the graph
# pdf(here("./figures/ran_effects.pdf"), width = 11, height = 7)
# g_re3
# dev.off()
```
