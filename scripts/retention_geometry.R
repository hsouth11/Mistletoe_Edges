...
a_35
a_20<- st_union(ed_20, rp_20) %>% st_area()/10000
a_20
...
ed_20_clearcut <- st_buffer(cb_line, 20) %>% 
  st_intersection(cb)
a_ed_20_clearcut <- st_area(ed_20_clearcut)/10000
#Compare these two numbers
a_ed_20_clearcut
a_20 #Approximately doubles it
a_20 - a_ed_20_clearcut
...
#within 20m of two retention patches. But from the plots, these are minimal,
#so its a good approximation
a_overlap <- st_area(overlap_20)/10000
a_overlap
...
a_inf <- ((st_area(rp_20)*0.3) + 
  (st_area(ed_20)*0.3) + (st_area(overlap_20)*0.3))/10000

#If just a clearcut
a_inf_clearcut <- a_ed_20_clearcut*0.3
...
rp_20 <- st_as_sf(rp_20)
ed_20 <- st_as_sf(ed_20)
...
group_reten <- bind_rows(
  ...
  rp_20 %>% mutate(category = "≤ 20 m"),
  ed_20 %>% mutate(category = "≤ 20 m")) %>% 
...
clearcut <- bind_rows(
  ...
  ed_20_clearcut %>% mutate(category = "≤ 20 m")) %>% 
  mutate(category = factor(category, levels = c("Mature forest", "> 35 m",
                                                "≤ 35 m", "≤ 20 m")),
         system = "Clearcut")
