...
could find a way to consider dead trees and Amabalis fir, the other HDM site,
which was only a significant host at one site (ph_2). 

NECESSARY TO DO
- How is amabilis fir being treated right now? It shouldn't have a crown vol
estimate so its sp should be 0, which means it is probably being filtered out. 
...
trees <- left_join(test, sl_target_rm, by = join_by(tree_id == tree1))
trees <- left_join(test, sl_target_rr, by = join_by(tree_id == tree1))
...
Now recreate the example above but applied across all trees at all sites. Start
by defining a function for interception:
```{r}
#Input requirements:
...
##seed load should have a value for all live Hw trees. If they 
...
#Create a temporary object with al lthe trees that should have values for sl
x <- trees %>% 
