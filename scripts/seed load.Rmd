...
p1 <- ggplot(sdisp, aes(x=dist_m_stem, y = p_seed)) + 
  ...
  annotate("text", x = 0, y = 0.4, 
           label = "Stem", hjust = 0, colour = "grey", size = 4) + 
  annotate("text", x = 3.855, y = 0.4, 
           label = "Crown edge", hjust = 0, colour = "grey", size = 4) +
  lims(y=c(0, 0.5),
       x=c(0, 25))+
  theme_classic() +
  labs(x = "Distance between trees (m)", 
       y = "Proportion of seed production") + 
...
```{r}
#Define palette for seed production variable
colour_sp <- c("#A6CEE3", brewer.pal(5, "YlOrRd"))

#Plot with lines between pairs 
p3 <- tmap_mode("plot") + 
  tm_shape(pair_lines_rm) + tm_lines(lty = 1) +
  tm_shape(test) + 
    tm_symbols(shape = "tree_type", 
               title.shape = "Tree type",
               col = "sp",
...
#Create some layers
x <- pair_fp_rm %>% filter(tree1 == "r1")
x1 <- test %>% filter(tree_id == "r1")
x2 <- test %>% filter(status %in% c("LS", "LL", "LF"), 
                      tree_type == "mature") %>% 
  mutate(crown_class_2 = as.character(crown_class_2)) %>% 
  mutate(crown_class_2 = factor(crown_class_2, levels = c("C", "I", "S")))
...
pdf("./figures/sl_workflow.pdf", width=11, height=15)
ggarrange(p1, p2_grob, p3_grob, p4_grob, p5_grob, 
...
tbl_sp_sl <- tbl_sp_sl %>% 
  ...
  select(tt_var, crown_class_2, combined) %>% 
