...
x <- mat.hwba %>% 
  group_by(site_id) %>% 
  summarise(mat.hwba_nstem_wb = sum(phf_site[broom_pa == "Y"]),
            mat.hwba_nstem_si = sum(phf_site[stem_pa == "Y"]))
...
mat_infection <- mat_infection %>% 
  mutate(mat.p_nstem_ha_inf_wb = mat.hwba_nstem_wb/mat.inf_nstem_ha,
         mat.p_nstem_ha_inf_si = mat.hwba_nstem_si/mat.inf_nstem_ha)

#Take a look
mat_infection %>% select(p.nstem_wb, p.nstem_si) %>% summary()
```
...
script. We will pull from that and from the paired one in the mature component 
...
inf_sum <- site %>% select(site_id)
x <- tran %>% 
  group_by(site_id) %>% 
  summarise(out_dist_inf = min(tr_leng_s))
inf_sum$out_inf_dist <- x$out_dist_inf

#Vars: infection perimeter and the percent of trees simulated
x <- reg_infection %>% 
  select(site_id, reg.infper_mean, reg.p_sim) 

x <- x %>% 
  select(site_id, reg.infper_mean, reg.p_sim) 
inf_sum <- left_join(inf_sum, x, by = "site_id")

#Vars: regen infection stats
x <- reg_infection %>% 
  select(site_id, reg.p_nstem_inf_15, reg.p_nstem_inf_50, reg.s_dmr_15,
         reg.s_dmr_50, reg.s_dmi_15, reg.s_dmi_50, reg.p_nstem_inf_wb_15,
         reg.p_nstem_inf_wb_50, reg.p_nstem_inf_si_15, reg.p_nstem_inf_si_50)
inf_sum <- left_join(inf_sum, x, by = "site_id")

#Vars: mature infection stats
x <- mat_infection %>% 
  select(site_id, mat.inf_ba_m2_ha, mat.p_nstem_inf, mat.s_dmr, mat.s_dmi, 
         mat.p_nstem_ha_inf_wb, mat.p_nstem_ha_inf_si)
inf_sum <- left_join(inf_sum, x, by = "site_id")
summary(inf_sum)

#Summarize each variable into mean, median and range
...
#Combine max and min into a range
inf_sum <- inf_sum %>% 
...
```{r}
#Table for thesis
# write_csv(inf_sum, "./tables/thesis_inf_stats.csv")
