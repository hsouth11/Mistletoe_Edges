...
p <- ggplot(mo_p2, aes(x = Month, y = pre_mm, fill = pre_type)) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
        ...
        legend.text = element_text(size = 16),
        legend.title = element_text(size = 20)) + 
  labs(x = "Month", y = "Precipitation (mm)", fill = "Type")

#Export this plot
# pdf(here("./figures/monthly_precip.pdf"), width = 11, height = 8.5)
# p
# dev.off()
...
p <- ggplot(mo_p, aes(x = Month, y = Prec_diff, color = site_id)) +
  ...
  theme(axis.text = element_text(size = 16),
        ...
        legend.text = element_text(size = 16),
        legend.title = element_text(size = 20)) + 
  labs(x = "Month", y = "Difference from mean (mm)", color = "Site")

# pdf(here("./figures/precip_diff.pdf"), width = 11, height = 8.5)
# p
# dev.off()
```
...
cs_sum <- site %>% select(site_id, age) %>% 
...
th <- th %>% 
  select(site_id, n_ht_r_hdm, ht_r_C, source_r_ht, n_ht_m_hdm, ht_m_C,
         source_m_ht)
head(th)

#Calculate height difference 
th <- th %>% 
  mutate(ht_diff_cd_hw = ht_m_C - ht_r_C)
head(th$ht_diff_cd_hw)

#Join this to the summary dataframe
cs_sum <- left_join(cs_sum, th, by = "site_id")

#Round height estimates to one decimal
cs_sum <- cs_sum %>% 
  mutate(across(c(ht_r_C, ht_m_C, ht_diff_cd_hw), ~ round(., 1)))
```
...
cs_sum <- cs_sum %>% 
  mutate(reg.nstem_ha = reg_cs$reg.nstem_ha,
         ...
         reg.p_stem_hw = 100*reg_cs$reg.p_stem_hw,
         mat.p_stem_hw = 100*mat_cs$mat.p_stem_hw) %>% 
  mutate(across(c(reg.nstem_ha, mat.nstem_ha), ~round(., 0)),
         across(c(reg.p_stem_hw, mat.p_stem_hw), ~round(., 1)),
         )
...
cs_sum <- cs_sum %>% 
  mutate(reg.ba_m2_ha = reg_cs$reg.ba_m2_ha,
         ...
         reg.p_ba_hw = 100*reg_cs$reg.p_ba_hw,
         mat.p_ba_hw = 100*mat_cs$mat.p_ba_hw) %>% 
  mutate(across(c(reg.ba_m2_ha, mat.ba_m2_ha), ~round(., 0)),
         across(c(reg.p_ba_hw, mat.p_ba_hw), ~round(., 1)),
         )
...
Amabalis fir (Ba) is another HDM host that was a significant portion of one site (ph_2). We are going to exclude it from the general summary stats and graphs but summarize infection in it at ph_2 to add to the text of the 
results section. 
...
mat.inf <- mature %>% 
  mutate(ba_m2_phf = ba_m2*phf_site) %>% 
  filter(spp %in% c("Hw", "Ba") & hdm_pa =="Y") %>% 
  group_by(site_id) %>% 
  summarise(mat.hw_inf_ba_m2_ha = sum(ba_m2_phf[spp == "Hw"]), 
            mat.hw_inf_nstem_ha = sum(phf_site[spp == "Hw"]),
            mat.hwba_inf_ba_m2_ha = sum(ba_m2_phf), 
...
#Including Ba and Hw
mat_infection %>% select(site_id, mat.nstem_ha, mat.hw_nstem_ha, 
...
Break down Hw BA and # of stems by site, presence/absence of HDM and dbh 
...
###Graph this breakdown
Break down Hw BA and # of stems (perha) by site, DMR and dbh  class. 
Then graph it. 
```{r}
mat.dmr_dbh <- mat.hw %>% 
...
p <- ggplot(mat.dmr_dbh, aes(x=dbh_bin2, y=nstem_ha, fill=dmr_f2)) + 
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
        ...
        legend.text = element_text(size = 16),
        legend.title = element_text(size = 20)) + 
  labs(x = "DBH bin (cm)", y = "Trees per ha", fill = "Adapted DMR")
...
x <- mat.hw %>% 
  ...
  group_by(site_id) %>% 
  summarise(mat.l_hw_nstem_ha = sum(phf_site), 
            mat.l_hw_nstem_wb = sum(phf_site[broom_pa == "Y"]),
            mat.l_hw_nstem_si = sum(phf_site[stem_pa == "Y"]))
  
#Join these back to results table
mat_infection <- left_join(mat_infection, x, by = "site_id")

#Calculate percent of live trees with brooming and stem infections
mat_infection <- mat_infection %>% 
  mutate(mat.p_l_hw_nstem_ha_wb = 100*mat.l_hw_nstem_wb/mat.l_hw_nstem_ha,
         mat.p_l_hw_nstem_ha_si = 100*mat.l_hw_nstem_si/mat.l_hw_nstem_ha)

#Take a look
mat_infection %>% select(mat.p_l_hw_nstem_ha_wb, 
                         mat.p_l_hw_nstem_ha_si) %>% summary()
```

##Stand DMR and DMI of each site
  - Stand DMR = mean DMR of all live Hw trees (a proxy of infection 
  severity at a site)
  - DMI = mean DMR of all live trees with DMR >=1 (a proxy of infection 
  severity where infection occurs). Note: using DMR >=1 is consistent with 
  other literature, that didn't include the IBLC infection class. In the 
  traditional application of the Hawksworth rating system, if a tree is
  infected, its DMR >=1. 
```{r}
mat.hw_s_dmr_dmi <- mat.hw %>% 
  filter(status %in% c("LS", "LL", "LF")) %>% 
  group_by(site_id) %>% 
  summarise(mat.hw_s_dmr = mean(dmr),
            mat.hw_s_dmi = mean(dmr[dmr >= 1]))

#Join these back to results table
mat_infection <- left_join(mat_infection, mat.hw_s_dmr_dmi, by = "site_id")

#Inspect them
mat_infection %>% select(site_id, mat.p_hw_nstem_inf, mat.hw_s_dmr, 
                         mat.hw_s_dmi)
```
...
infection variables. 
  - Variables: 
    - basal area infected (live and dead Hw)
    - percent live and dead Hw stems infected 
    - average DMR (live Hw)
    - DMI (live Hw dmr>=1)
    - Percent of live Hw stems with brooms
    - Percent of live Hw stems with stem infection
  - Structure:
    - Rows: variables
...
mat_inf2 <- mat_infection %>% 
  select(mat.hw_inf_ba_m2_ha, mat.p_hw_nstem_inf, mat.hw_s_dmr, mat.hw_s_dmi, 
         mat.p_l_hw_nstem_ha_wb, mat.p_l_hw_nstem_ha_si)
...
infection at that site. Stats:
  -Site basal area/ha
  -Site stem density/ha
  -Ba basal area/ha
  -Ba stem density/ha
  -Ba basal area/ha infected
  -Ba stem density/ha infected
  -Percent of site basal area made up of Ba
  -Percent of site stems made up of Ba
  -Percent of Ba basal area infected
  -Percent of Ba stems infected
  -Average Ba DMR
  -Average Ba DMI
Compare all these things to Hw, as a reference point. 

Note: for clarity, Ba is abbreviated with af (Amabalis Fir) to avoid confusion
with basal area. 
```{r}
...
ph2.mat <- mature %>% 
  ...
  summarise(nstem_ha = sum(phf_site),
            ...
            af_inf_nstem_ha = sum(phf_site[spp == "Ba" & hdm_pa == "Y"]),
            ba_m2_ha = sum(ba_m2_phf),
            af_ba_m2_ha = sum(ba_m2_phf[spp == "Ba"]),
            af_inf_ba_m2_ha = sum(ba_m2_phf[spp == "Ba" & hdm_pa == "Y"]),
  ...
  mutate(p_nstem_af = 100*af_nstem_ha/nstem_ha,
         ...
         p_af_nstem_inf = 100*af_inf_nstem_ha/af_nstem_ha,
         p_af_ba_inf = 100*af_inf_ba_m2_ha/af_ba_m2_ha)
...
mat_cs %>% 
  filter(site_id=="ph_2") %>% 
  select(mat.p_stem_hw, mat.p_ba_hw)
ph2.mat %>% select(p_nstem_af, p_ba_af)

##How much of the Hw and Ba at each site are infected?
###Infection rates higher in Hw
mat_infection %>% filter(site_id == "ph_2") %>% 
  select(mat.p_hw_nstem_inf, mat.p_hw_ba_inf)
ph2.mat %>% select(p_af_nstem_inf, p_af_ba_inf)

#What is the mean DMR and DMI of Hw and Ba?
###Infection more severe in Hw than Ba
mat_infection %>% filter(site_id == "ph_2") %>% 
  select(mat.hw_s_dmr, mat.hw_s_dmi)
ph2.mat %>% select(s.af_dmr, s.af_dmi)
...
across live and dead trees hemlocl, while the second looks just at live 
hemlock. 
...
test <- reg.hw_50 %>% 
  filter(site_id == "ph_2" & dist_y_h_bin == "(15,20]")
...
p <- ggplot(reg.dmr_disty, aes(x=dist_y_h_bin, y=nstem_ha, fill=dmr_f2)) + 
  geom_bar(position=position_stack(reverse = TRUE), stat="identity") +
  geom_text(aes(y = lab_pos_nstem, label = tr_end, vjust = -.5), 
            size = 5) +
  theme_classic() +
  facet_wrap(vars(site_id)) + 
  scale_fill_manual(values = colors_dmr_f2) + 
...
combination: percent of Hw stems infected, average Hw DMR and DMI and the 
percent infected Hw with brooms and stem infections. Summarize them at the
...
x <- regen_15_l %>% 
  ...
  summarise(nstem_ha_l_hw = sum(phf_site),
            ...
            nstem_ha_l_hw_wb = sum(phf_site[broom_pa == "Y"]),
            nstem_ha_l_hw_si = sum(phf_site[stem_pa == "Y"])) %>% 
  mutate(p_sim = 100*nstem_ha_l_hw_sim/nstem_ha_l_hw, 
         p_l_hw_inf = 100*nstem_ha_l_hw_inf/nstem_ha_l_hw,
         p_l_hw_wb = 100*nstem_ha_l_hw_wb/nstem_ha_l_hw,
         p_l_hw_si = 100*nstem_ha_l_hw_si/nstem_ha_l_hw) %>% 
...
reg.vpos <- reg.vpos %>% 
  ...
  mutate(ct = factor(ct, levels = c("upper", "middle", "lower")),
         crown_class = factor(crown_class, c("D", "C", "I", "S")))
...
##Percent of trees with growth impacts
Growth impacts start to occur at DMR >=3. See what percent of codominant Hw
at each site are past that threshold. 
```{r}
#Within 15m calculate the number of dominant/codominant live Hw and the 
#percent of those with DMR>=3
dmr3_15 <- regen_15_l %>% 
  filter(crown_class_2 == "C") %>% 
  group_by(site_id) %>% 
  summarise(n_dc_hw = n(),
            n_dc_hw_dmr3 = sum(dmr >= 3)) %>% 
  mutate(p_dmr3 = 100*n_dc_hw_dmr3/n_dc_hw)
...
site, mirroring the stats created for the mature component. In thesis, chose
to report incidence as a percent of live Hw, so do that here for Ba too for
consistency. Use the 0-15m interval. Stats:
  -Site basal area/ha
  -Site stem density/ha
  -Ba basal area/ha
  -Ba stem density/ha
  -Live Ba stem density/ha
  -Ba basal area/ha infected (live and dead trees)
  -Ba stem density/ha infected (live and dead trees)
  -Live Ba stem density/ha infected
  -Percent of site basal area made up of Ba
  -Percent of site stems made up of Ba
  -Percent of Ba basal area infected
  -Percent of Ba stem infected (live and dead)
  -Percent of live Ba stems infected
  -Average Ba DMR
  -Average Ba DMI
Compare all these things to Hw, as a reference point. 
...
ph2.reg15 <- regen_15 %>% 
  ...
  summarise(nstem_ha = sum(phf_site),
            af_nstem_ha = sum(phf_site[spp == "Ba"]),
            af_l_nstem_ha = sum(phf_site[spp == "Ba" & status %in% live_stat]),
            af_inf_nstem_ha = sum(phf_site[spp == "Ba" & hdm_pa == "Y"]),
            af_l_inf_nstem_ha = sum(phf_site[spp == "Ba" & hdm_pa == "Y" &
                                               status %in% live_stat]),
            ba_m2_ha = sum(ba_m2_phf),
            af_ba_m2_ha = sum(ba_m2_phf[spp == "Ba"]),
            af_inf_ba_m2_ha = sum(ba_m2_phf[spp == "Ba" & hdm_pa == "Y"]),
  ...
  mutate(p_nstem_af = 100*af_nstem_ha/nstem_ha,
         ...
         p_af_nstem_inf = 100*af_inf_nstem_ha/af_nstem_ha,
         p_af_ba_inf = 100*af_inf_ba_m2_ha/af_ba_m2_ha,
         p_l_af_stem_inf = 100*af_l_inf_nstem_ha/af_l_nstem_ha)
...
reg_cs %>% 
  filter(site_id=="ph_2") %>% 
  select(reg.p_stem_hw, reg.p_ba_hw)
ph2.reg15 %>% select(p_nstem_af, p_ba_af)

##How much of the Hw and Ba (live and dead trees) are infected?
###Infection much higher in hemlock 
reg_i_all %>% filter(site_id == "ph_2") %>% 
  select(reg.p_ba_m2_inf_15, reg.p_nstem_inf_15)
ph2.reg15 %>% select(p_af_nstem_inf, p_af_ba_inf)

##How much of the live Hw and Ba are infected?
###Hemlock, much higher rates of inf
regen_15_l %>%   
  filter(site_id == "ph_2" & spp == "Hw") %>% 
  summarise(hw_nstem = sum(phf_site),
            hw_inf_nstem = sum(phf_site[hdm_pa == "Y"])) %>% 
  mutate(p_l_hw_nstem_inf = 100*hw_inf_nstem/hw_nstem)
ph2.reg15 %>% select(p_l_af_stem_inf)
...
regen_15_l %>% 
  filter(site_id == "ph_2" &
           spp == "Hw") %>% 
  summarise(hw_s_dmr = mean(dmr),
            hw_s_dmi = mean(dmr[dmr >= 1]))
ph2.reg15 %>% select(s.af_dmr, s.af_dmi)
```