...
#Export this plot
# ggsave(here("./figures/climate_vars.svg"), plot = p, device = "svg",
...
#Including Ba and Hw
mat_i_ld_hw %>% select(site_id, mat.nstem_ha, mat.hw_nstem_ha, 
...
mat_i_ld_hw <- left_join(mat_i_ld_hw, mat.dmr, by = "site_id")
```
...
x <- regen_15_l %>% 
  ...
  select(-c(nstem_ha_l_hw, nstem_ha_l_hw_inf, nstem_ha_l_hw_wb,
            nstem_ha_l_hw_si, nstem_ha_l_hw_dmr3)) %>% 
  mutate(sec = "0_15")
y <- x1 %>% filter(crown_class_2 == "C") %>% arrange(p_l_hw_dmr3)
#For 0-50m 
x1 <- regen_50_l %>% 
  group_by(site_id, crown_class_2) %>% 
  summarise(nstem_ha_l_hw = sum(phf_site),
...
x2 <- x2 %>% 
  mutate(metric = case_match(metric,
                             ...
                             "hw_s_dmi" ~ "DMI",
                             "p_l_hw_inf" ~ "% trees infected",
                             "p_l_hw_wb" ~ "% brooming",
                             "p_l_hw_si" ~ "% stem infections",
                             "p_l_hw_dmr3" ~ "% trees DMR ≥ 3")) %>% 
  mutate(metric = factor(metric, levels = c("% trees infected", 
                                            "% trees DMR ≥ 3",
                                            "% brooming",
                                            "% stem infections", 
                                            "Average DMR",
...
p3 <- ggarrange(p1, p2, ncol = 2, common.legend = T, align = "h",
                widths = c(1.5, 1), labels = "AUTO", vjust = 1.2, 
