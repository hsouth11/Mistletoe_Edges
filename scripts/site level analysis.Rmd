...
mat.dmr_dbh <- mat.hwba %>% group_by(site_id, dmr_f2, dbh_bin2) %>%
  summarise(ba_m2 = sum(ba_m2), nstem = n()) %>% 
  complete(dbh_bin2, fill=list(ba_m2=0, nstem=0))
...
#Calculate the proportion of Hw infected within 15m and total
reg_infection <- reg_infection %>% 
  mutate(reg.p_inf_ba_m2 = reg.inf_ba_m2/reg.hw_ba_m2,
         reg.p_inf_nstem = reg.inf_nstem/reg.hw_nstem,
         reg.p_inf_ba_m2_15 = reg.inf_ba_m2_15/reg.hw_ba_m2_15,
...
reg_infection %>% select(site_id, reg.s_dmr_15, reg.s_dmi_15) %>% summary()
...
ggplot(reg.dmr_disty, aes(x=dist_y_h_bin, y=nstem, fill=dmr_f2)) + 
  geom_bar(position=position_stack(reverse = TRUE), stat="identity") +
  theme_classic() +
  facet_wrap(vars(id_bec_age_f)) + 
  scale_fill_manual(values = colors_dmr_f2) + 
...
p <- ggplot(reg.dmr_disty, aes(x=dist_y_h_bin, y=ba_m2, fill=dmr_f2)) + 
  geom_bar(position=position_stack(reverse = TRUE), stat="identity") +
  theme_classic() +
  facet_wrap(vars(id_bec_age_f)) + 
  scale_fill_manual(values = colors_dmr_f2, 
...
#WIFDWC PLOT
p <- ggplot((reg.hw %>% filter(status %in% c("LS", "LL", "LF"))), 
       aes(x=dist_y_h, y=dmr)) + 
  theme_classic() +
  scale_y_continuous(limits = c(-0.1, 6.1),
                     breaks = seq(0, 6, by =1)) +
  geom_jitter(height = 0.1, width = 0) +
  labs(x="Distance from edge (m)", y = "DMR") +
  theme(axis.text = element_text(size = 20),
        axis.title = element_text(size = 28))
?scale_y_continuous
#Export this plot
pdf(here("./figures/reg_dmr_disty.pdf"), width = 11, height = 4)
p
dev.off()
...
```{r}
#Assembling a summary table of site level results for WIFDWC
#Variables: 
#descriptors: site_id, bec, regen age 
#regen: infection perimeter, percent infection (within 15 and total), stand DMR 
#(within 15 and total) 
#mature: BA infected, stand DMR
wifdwc <- site %>% select (site_id, bec, age)
wifdwc <- left_join(wifdwc, select(reg_infection, site_id, reg.infper_m, 
                                 reg.p_inf_ba_m2_15, reg.p_inf_ba_m2,
                                 reg.s_dmr, 
                                 reg.s_dmr_15, 
                                 reg.s_dmi,
                                 reg.s_dmi_15), 
                    by = "site_id")
wifdwc <- left_join(wifdwc, select(mat_infection, site_id, mat.inf_ba_m2,
                                   mat.s_dmr,
                                   mat.s_dmi),
                    by = "site_id")
write_csv(wifdwc, here("./tables/wifdwc_2024.csv"))
```
