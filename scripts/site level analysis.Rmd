...
reg_ba_nstem <- reg_ba_nstem %>% mutate(reg.tot_ba_m2ha = reg.tot_ba_m2/0.0225, #Add a columns with per hectare numbers for easy reading
                                     reg.nstem_ha = reg.nstem/0.0225) 
##Mature
...
mat_ba_nstem <- mat_ba_nstem %>% mutate(mat.tot_ba_m2ha = mat.tot_ba_m2/0.055, #Add a columns with per hectare numbers for easy reading
                                     mat.nstem_ha = mat.nstem/0.055) 
...
reg_dbh <- regen_15 %>% group_by(id_bec_age_f) %>% 
...
mat_dbh <- mature_comp %>% group_by(id_bec_age_f) %>% 
...
reg_hw_dbh <- regen_15 %>% filter(spp=="Hw") %>% group_by(id_bec_age_f) %>% 
...
mat_hw_dbh <- mature_comp %>% filter(spp=="Hw") %>% group_by(id_bec_age_f) %>% 
...
site <- left_join(site, reg_dbh, by="id_bec_age_f")
site <- left_join(site, mat_dbh, by="id_bec_age_f")
site <- left_join(site, reg_hw_dbh, by="id_bec_age_f")
site <- left_join(site, mat_hw_dbh, by="id_bec_age_f")
####################
...
###Regen
reg.hwcwfdba_ba_m2 <- reg_struc %>% filter(spp %in% c("Hw", "Cw", "Fd", "Ba")) %>% group_by(id_bec_age_f) %>% 
  summarise(reg_hwcwfdba_ba_m2 = sum(ba_m2)) #calculate total BA of Hw, Cw, Fd and Ba at each site
reg.hwcwfdba_nstem <- reg_struc %>% filter(spp %in% c("Hw", "Cw", "Fd", "Ba")) %>% group_by(id_bec_age_f) %>% 
  summarise(reg_hwcwfdba_nstem = sum(nstem)) #calculate total # of stems of Hw, Cw, Fd and Ba at each site
reg.hw_ba_m2 <- reg_struc %>% filter(spp == "Hw") %>% group_by(id_bec_age_f) %>% 
  summarise(reg.hw_ba_m2 = sum(ba_m2)) #calculate BA of Hw at each site
reg.hw_nstem <- reg_struc %>% filter(spp == "Hw") %>% group_by(id_bec_age_f) %>% 
...
site <- site %>% mutate(reg.p_ba_hwcwfdba = reg_hwcwfdba_ba_m2/reg.tot_ba_m2, #proportion BA Hw, Fd, Cw Ba
                        reg.p_stem_hwcwfdba = reg_hwcwfdba_nstem/reg.nstem, #proportion stems Hw, Fd, Cw Ba
                        reg.p_ba_hw = reg.hw_ba_m2/reg.tot_ba_m2, #proportion BA Hw
                        reg.p_stem_hw = reg.hw_nstem/reg.nstem) #proportion stems Hw
###Mature
mat.hwcwfdba_ba_m2 <- mat_struc %>% filter(spp %in% c("Hw", "Cw", "Fd", "Ba")) %>% group_by(id_bec_age_f) %>% 
  summarise(mat_hwcwfdba_ba_m2 = sum(ba_m2)) #calculate total BA of just Hw, Cw, Fd and Ba
mat.hwcwfdba_nstem <- mat_struc %>% filter(spp %in% c("Hw", "Cw", "Fd", "Ba")) %>% group_by(id_bec_age_f) %>% 
  summarise(mat_hwcwfdba_nstem = sum(nstem)) #calculate total # of stems of Hw, Cw, Fd and Ba at each site
mat.hw_ba_m2 <- mat_struc %>% filter(spp == "Hw") %>% group_by(id_bec_age_f) %>% 
  summarise(mat.hw_ba_m2 = sum(ba_m2)) #calculate BA of Hw
mat.hw_nstem <- mat_struc %>% filter(spp == "Hw") %>% group_by(id_bec_age_f) %>% 
...
site <- site %>% mutate(mat.p_ba_hwcwfdba = mat_hwcwfdba_ba_m2/mat.tot_ba_m2, #proportion BA Hw, Fd, Cw Ba
                        mat.p_stem_hwcwfdba = mat_hwcwfdba_nstem/mat.nstem, #proportion stems Hw, Fd, Cw Ba
...
site %>% select(id_bec_age_f, reg.tot_ba_m2, reg_hwcwfdba_ba_m2, reg.hw_ba_m2, p.reg_ba_hwcwfdba, p.reg_ba_hw) %>% 
  arrange(reg.tot_ba_m2)
site %>% select(id_bec_age_f, mat.tot_ba_m2, mat_hwcwfdba_ba_m2, mat.hw_ba_m2, p.mat_ba_hwcwfdba, p.mat_ba_hw) %>% 
  arrange(mat.tot_ba_m2)

##Filter
reg_struc <- filter(reg_struc, spp %in% c("Hw", "Cw", "Fd", "Ba"))
mat_struc <- filter(mat_struc, spp %in% c("Hw", "Cw", "Fd", "Ba"))
...
ggplot(reg_struc, aes(x=dbh_bin, y=nstem, fill=spp)) + geom_bar(position="stack", stat="identity") + 
...
ggplot(mat_struc, aes(x=dbh_bin, y=nstem, fill=spp)) + geom_bar(position="stack", stat="identity") + 
  facet_wrap(vars(id_bec_age_f)) + scale_fill_manual(values = colors_spp)
...
site <- site %>% mutate(p_mat_hw_ba_inf = mat_inf_ba_m2/mat.hw_ba_m2) #calculate the proportion of Hw basal area that's infected at each site

#Create a summary table. 
site %>% select(id_bec_age_f, mat.hw_ba_m2, mat_inf_ba_m2, p_mat_hw_ba_inf) %>% #summary table sorted by proportion of BA infected
...
ggplot(mat_hdmpa, aes(x=dbh_bin, y=nstem, fill=hdm_pa)) + geom_bar(position="stack", stat="identity") + 
...
ggplot(mat_dmr, aes(x=dbh_bin, y=nstem, fill=dmr_f2)) + geom_bar(position="stack", stat="identity") + 
  facet_wrap(vars(id_bec_age_f)) + scale_fill_manual(values = colors_dmr_f2)
...
site <- site %>% mutate(p_reg_15_ba_inf = reg_15_inf_ba_m2/reg.tot_ba_m2)

#Create a summary table. 
site %>% select(id_bec_age_f, reg.hw_ba_m2, reg_inf_ba_m2, reg_15_inf_ba_m2, p_reg_15_ba_inf) %>% 
...
ggplot(reg_hdmpa, aes(x=dist_y_h_bin, y=nstem, fill=hdm_pa)) + geom_bar(position="stack", stat="identity") + 
...
##Plot nstem and basal area across distance from the edge, coloured by presence/absence of hdm infection
ggplot(reg_dmr, aes(x=dist_y_h_bin, y=nstem, fill=dmr_f2)) + geom_bar(position="stack", stat="identity") + 
  facet_wrap(vars(id_bec_age_f)) + scale_fill_manual(values = colors_dmr_f2)
