...
(which  extends past the zone of influence and will contain simulated trees for 
...
##Brooming and stem infections
Calculate the proportion of infected stems with brooms and stem infections
```{r}
#0-15m
reg.wb_si_15 <- reg.hw_15 %>% 
  group_by(site_id) %>% 
  summarise(reg.nstem_ha_wb_15 = sum(phf_site[broom_pa == "Y"]),
            reg.nstem_ha_si_15 = sum(phf_site[stem_pa == "Y"])) %>% 
  complete(site_id, 
           fill=list(reg.nstem_ha_wb_15=0, 
                     reg.nstem_ha_si_15=0))

#0-50m
reg.wb_si_50 <- reg.hw_50 %>% 
  group_by(site_id) %>% 
  summarise(reg.nstem_ha_wb_50 = sum(phf_site[broom_pa == "Y"]),
            reg.nstem_ha_si_50 = sum(phf_site[stem_pa == "Y"])) %>% 
    complete(site_id, 
           fill=list(reg.nstem_ha_wb_50=0, 
                     reg.nstem_ha_si_50=0))

#Join these back to results table
reg_infection <- left_join(reg_infection, reg.wb_si_15, by = "site_id")
reg_infection <- left_join(reg_infection, reg.wb_si_50, by = "site_id")

#Calculate proportion of trees with brooming
reg_infection <- reg_infection %>% 
  mutate(reg.p_nstem_inf_wb_15 = reg.nstem_ha_wb_15/reg.nstem_ha_hw_inf_15,
         reg.p_nstem_inf_si_15 = reg.nstem_ha_si_15/reg.nstem_ha_hw_inf_15,
         reg.p_nstem_inf_wb_50 = reg.nstem_ha_wb_50/reg.nstem_ha_hw_inf_50,
         reg.p_nstem_inf_si_50 = reg.nstem_ha_si_50/reg.nstem_ha_hw_inf_50
         )

#Take a look
reg_infection %>% 
  select(contains(c("p_nstem_inf_wb", "p_nstem_inf_si"))) %>% 
  summary()
```

##Summary table for thesis
Create a summary table for thesis. This recreates some of the work above but 
when comparing to other papers, it became important to have infection metrics
broken down by crown class. The code above combines all trees across crown 
classes. Most other papers report infection metrics for the
something analagous to the codominant layer so need this for comparison. 

Going to filter summary to live Hw trees. This will be much simpler to 
summarize and aligns with what is in the ordinal models. 

Variables:
  - Outer infected tree (m)
  - Infection perimeter (m)
...
```{r}
regen_15_l <- regen_15 %>% 
  filter(spp == "Hw" & status %in% c("LS", "LL", "LF") & 
           sim_tree == "N")
regen_50_l <- regen_50 %>% 
  filter(spp == "Hw" & status %in% c("LS", "LL", "LF"))
```
...
x$sec <- "0_15"
x1$sec <- "0_50"
reg_inf2 <- bind_rows(x, x1) %>% 
  relocate(sec, .before = everything())
```

That is super. Now repeat this for the same variables but ingnoring crown 
...
x <- regen_15_l %>% 
  group_by(site_id) %>% 
  summarise(ip = if_else(sum(hdm_pa == "Y")>0,
                      quantile(dist_y_h[hdm_pa == "Y"], probs = 0.9),
                                 0),
            nstem_ha_l_hw = sum(phf_site),
...
reg_inf2 <- bind_rows(reg_inf2, x, x1)
```

Next calculate the the outer infected tree at teach site. 
```{r}
#Start by getting the outer infected tree at each site
x <- regen %>% 
  mutate(resid = if_else(tree_id %in% resid_rm, "Y", "N")) %>% 
  filter(resid == "N" & spp == "Hw" & status %in% c("LS", "LL", "LF") &
           hdm_pa == "Y") %>% 
  group_by(site_id) %>% 
  summarise(out_inf_dist = max(dist_y_h)) %>% 
  ungroup() %>% 
  summarise(mean_od = mean(out_inf_dist),
            median_od = median(out_inf_dist),
            min_od = min(out_inf_dist),
            max_od = max(out_inf_dist)) %>%
  mutate(across(where(is.numeric), ~round(.x, 2))) %>% 
  mutate(range = paste0("[", min_od, "-", max_od, "]")) %>% 
  select(-c(min_od), max_od)

#Join this back to the working dataframe
x <- x %>% 
  mutate(sec = "all",
         crown_class_2 = "all")
reg_inf2 <- full_join(reg_inf2, x)
```
...
# write_csv(reg_inf2, here("./tables/reg_infection_thesis.csv"))
```
