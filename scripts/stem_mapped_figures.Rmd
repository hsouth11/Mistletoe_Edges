...
site <- read_csv('./data/cleaned/site data_8Apr2024.csv')
```
...
trees <- trees %>% mutate(across(c(site_id, spp, status, hdm_pa, b_lc, broom_pa, 
                                   broom_pos, broom_stem, crown_class, crown_cond, outside_10, 
...
#Up to 6 lines/transect, 2pts/line, 8 sites, 3 transects/site
# #pts = 3*8*6*2 = 288 pts
max(tran$tr_leng) #find longest transect length. 32.8m (cr_1)
# Create new fake points
dist_x <- rep(c(-2.5, 2.5), 3*8*6) #define vector of x coords
dist_y <- rep(c(5, 5, 10, 10, 15, 15, 20, 20, 25, 25, 30, 30), 3*8) #define vector of y coordinates
dist_y <- sort(dist_y)
ids <- sample(1:500, 144, replace = FALSE) #generate 144 random unique ids
ids <- sort(rep(ids, 2))
...
png("./figures/trees_2May2024.png", width=3300, height=2550, units="px", res = 300)
tmap_mode("plot")
