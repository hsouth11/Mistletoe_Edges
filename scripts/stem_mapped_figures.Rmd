...
rot_points <- rbind(select(trees_rot, site_id), #combine trees_rot and trimb_rot datasets
                    select(tran_pts, site_id))
...
?st_cast
#Create seperate layers for start points and end points
start_pts <- trimb_rot %>% filter(pt_type=="r adj tran start")
end_pts <- trimb_rot %>% filter(pt_type=="tran end")
end_pts <- left_join(end_pts, select(tran, transect_id, tr_leng), by=c("pt_id"="transect_id"))
...
tm_shape(edge_lines) + tm_lines(col="blue") + #add edge lines
tm_shape(tran_lines) + tm_polygons(alpha=0.75) + tm_text("tr_leng", ymod = 1.7) + #add transects
tm_shape(tran_pts) + tm_symbols(col="blue") + #transect start points
tm_shape(trees_rot) + tm_symbols(size = "dbh", col="dmr") + #add tree points
tm_legend(legend.outside=TRUE, legend.outside.position="bottom", legend.position = c(0.4, 0.1), legend.stack="horizontal")


?tm_text
?tm_symbols
#Plot the unrotated one:
tran_pts <- trimb %>% #filter to just transect starts and ends
  filter(pt_type %in% c("adj tran start", "tran end"))
tran_line_att <- tran_pts %>% st_drop_geometry() %>% #save attributes
  filter(pt_type=="adj tran start") %>% select(Comment, pt_id, site_id)
tran_lines <- tran_pts %>% group_by(pt_id) %>% # draw lines between start and end
  summarize() %>% st_cast("LINESTRING")
tran_lines <- st_buffer(tran_lines, dist = 2.5, endCapStyle = 'FLAT') # buffer
tran_lines <- left_join(tran_lines, tran_line_att, by="pt_id")
...
tm_shape(tran_pts) + tm_symbols(col="red") +
tm_shape(trees) + tm_symbols(size = "dbh", col="dmr") +
tm_facets(by="site_id", free.scales = FALSE)
?tm_facets()
st-buffer?


#Make a layer for transect edges: 
?st_buffer()
levels(mi2_trimb$pt_type) #check values of pt_type field
tran_lines <- mi2_trimb %>% #filter to just transect starts and ends
  filter(pt_type %in% c("adj tran start", "tran end"))
tran_lines <- tran_lines %>% group_by(pt_id) %>% summarize() %>% st_cast("LINESTRING")
tran_lines <- st_buffer(tran_lines, dist = 2.75, endCapStyle = 'FLAT')
#Trees layer: 
tmap_mode("plot")
tm_shape(mi2_trees) + tm_symbols(size = "dbh", col="dmr") + tm_text(text = "tree_id_new")+ tm_shape(tran_lines) + tm_polygons(alpha=0.75) 

x <- trees %>% filter(tree_id_new %in% c("r399", "r404", "r403")) %>% select(dist_x, dist_y)
st_length(tran_lines)
?tm_polygons()
```
