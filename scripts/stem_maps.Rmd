...
trees <- trees %>% 
  mutate(across(c(site_id, spp, status, hdm_pa, b_lc, broom_pa, broom_pos, 
                  ...
                  assessed_by, tree_type, tree_type_2, dmr_f), ~ as.factor(.)))
...
#Object 3: Footprints of each site
...
#Object 4 and 5: transect start/end points and footprints
...
#Add site id and factor with site_id, bec and age to each of the objects
...
#Define shape palette for tree_type_2
levels(trees_rot$tree_type_2)
tt2_shape <- c(16, 16, 1)

# Plot it all together
# pdf("./figures/trees_13Oct2024.pdf", width=11, height=8.5)
tmap_mode("plot") +
  #Define map extent using site footprints
  tm_shape(adjusted_fp_rot, is.master = TRUE) +
    tm_facets(by = "id_bec_ag_f", free.scales = FALSE, 
              free.coords = TRUE, ncol = 4) +
...
#Seed load addition
#Pull out an object where seed low is defined. 
#These are all live, Hw, regen, trees. Going to plot two versions of this,
#one coloured by seed load and one by dmr. 
trees_sl <- trees_rot %>% 
...
#Pull out another object that is live, mature Hw trees where sp is defined
trees_sp <- trees_rot %>% 
  filter(tree_type == "mature" & sp > 0)

# pdf("./figures/trees_sl_13Oct2024.pdf", width=11, height=8.5)
tmap_mode("plot") +
  #Define map extent using site footprints
  tm_shape(adjusted_fp_rot, is.master = TRUE) +
    tm_facets(by = "id_bec_ag_f", free.scales = FALSE, 
